...
async def lifespan(app: FastAPI):
    ...
    async with httpx.AsyncClient(
        ...
        with closing(sqlite3.connect(database)) as conn:
            ...
            # Apply any pending schema migrations. Blocks startup until
            # complete; for a large existing archive, the v2 migration
            # (per-session message hash dictionary) can take ~30s and prints
            # progress to stderr so users running `uvx cctape` interactively
            # know why startup is slow on first launch after upgrade.
            run_migrations(conn)
